...
public interface DtCooperationRepository extends DynamicJpaRepository<DtCooperation, Long>, DtCooperationRepositoryCustom{
     ...
     */
    @Query(value =" select t,o from com.openjava.datatag.tagmodel.domain.DtTaggingModel t, DtCooperation o where t.taggingModelId=o.taggmId \n" +
            "and (o.cooUser=:userId or :userId is null) and (t.modelName like '%' || :keyWord || '%' or :keyWord is null) and (o.taggmId=:taggmId or :taggmId is null) and (t.runState=:runState or :runState is null) order by t.modifyTime desc"
            )
    Page<?> findPageUserModelByUserId(@Param("userId")Long userId ,@Param("keyWord")String keyWord, @Param("taggmId")Long taggmId ,@Param("runState")Long runState ,Pageable pageable);
    /**
     *根据模型Id查找该用户要协作的字段记录集
     */
    @Query(value ="select t.*,o.ID,o.COO_USER,o.cooFieldId,o.USE_TAG_GROUP,o.TAG_COL_ID,(case when o.COO_USER=:userId and t.SHOW_COL=o.TAG_COL_NAME then '1' else '0' end ) IsCooField from\n" +
            ...
            "where TAGGING_MODEL_ID=:modelId and IS_DELETED !=1"
            ,nativeQuery = true)
    ...
    @Query(value ="select * from DT_TAG_GROUP t where t.ID in(\n" +
            ...
            "where l.TAG_COL_ID=:colField and o.COO_USER=:userId and o.TAGGM_ID=:modelId)"
            ,nativeQuery = true)
    List<Map<String, String>> findCurrentUserTagGroup(@Param("userId")Long userId, @Param("modelId") Long modelId, @Param("colField") Long colField);
