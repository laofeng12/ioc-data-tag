package com.openjava.datatag.utils.jdbc.dataprovider.aggregator.jvm;
...
public class JvmAggregator extends InnerAggregator {
    ...
    public AggregateResult queryAggData(AggConfig config) throws Exception {
        ...
        for (String[] row : result) {
            IntStream.range(0, dimSize).forEach(d -> {
                if (row[d] == null) row[d] = DataProvider.NULL_STRING;
            });
        }
    ...
    public AggregateResult queryTableData(String tableName, Pageable pageable) throws Exception {
        return null;
    }
    ...
    private class Filter {
        ...
        public boolean filter(String[] row) {
            boolean result = ruleList.stream().map(rule -> DataProvider.separateNull(rule)).allMatch(rule ->
