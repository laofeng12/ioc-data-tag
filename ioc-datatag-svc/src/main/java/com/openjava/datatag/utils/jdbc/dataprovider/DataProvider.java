...
import com.openjava.datatag.utils.jdbc.dataprovider.config.CompositeConfig;
import com.openjava.datatag.utils.jdbc.dataprovider.config.ConfigComponent;
import com.openjava.datatag.utils.jdbc.dataprovider.config.DimensionConfig;
import com.openjava.datatag.utils.jdbc.dataprovider.expression.NowFunction;
...
import com.openjava.datatag.utils.jdbc.dataprovider.dto.NaturalOrderComparator;
...
public abstract class DataProvider {
   ...
   /* @Autowired
    ...
    public final JdbcAggregateResult getAggDataByJdbcTemplate(AggConfig ac, boolean reload) throws Exception {
        evalValueExpression(ac);
        if (isDataSourceAggInstance()) {
            return ((Aggregatable) this).queryAggDataByJdbcTemplate(ac);
        } else {
            checkAndLoad(reload);
            return innerAggregator.queryAggDataByJdbcTemplate(ac);
        }
    }
    public final List<String> getFilter(String filter, boolean reload) throws Exception {
        if (isDataSourceAggInstance()) {
            return ((Aggregatable) this).queryFilterData(filter);
        } else {
            checkAndLoad(reload);
            return innerAggregator.queryFilterData(filter);
        }
    }

    public final AggregateResult getTableData(String columns, String tableName, Pageable pageable) throws Exception {
        return ((Aggregatable) this).queryTableData(columns, tableName, pageable);
    }
