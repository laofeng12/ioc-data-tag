    ...
    */
    ...
    public static Integer getAge(String dateOfBirthStr) {
    	...
    	try {
	        ...
	        Date dateOfBirth = myFormatter.parse(dateOfBirthStr);
    ...
    /**
     * 计算工龄
     * @param dateOfBirth 格式为：yyyyMMdd或者yyyyMM
     * @return
     */
    public static Double getWorkingAge(String dateOfBirthStr) {
    	...
    	try {
    		...
    		dateOfBirth = myFormatter.parse(dateOfBirthStr);
    	} catch (ParseException e) {
    		try {
				...
				try {
					...
					try {
						myFormatter = new SimpleDateFormat("yyyy-MM");
						dateOfBirth = myFormatter.parse(dateOfBirthStr);
